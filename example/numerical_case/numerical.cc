...
double prob_1_Rosenbrock(double* x) {
  x[2] = (100 * (x[1] - x[0] * x[0]) * (x[1] - x[0] * x[0]) +
          (1 - x[0]) * (1 - x[0]));
  return x[2];
}
double prob_4(double* x) {
  x[3] = x[0] + sin(5 * x[1]) - x[1] * x[1] + cos(7 * x[0]) - x[0] * x[1] +
         x[1] * x[2] - x[0] * x[2];
  x[4] = 2 * x[0] - 7 * x[1];
  x[5] = 3 * x[1] * x[1] - x[2];
  return x[3];
...
double prob_2_G_9(double* x) {
  x[8] = -(2 * pow(x[0], 2) + 3 * pow(x[1], 4) + x[2] + 4 * pow(x[3], 2) +
           5 * x[4] - 127);
  x[9] = -(7 * x[0] + 3 * x[1] + 10 * pow(x[2], 2) + x[3] - x[4] - 282);
  x[10] = -(23 * x[0] + pow(x[1], 2) + 6 * pow(x[5], 2) - 8 * x[6] - 196);
  x[11] = -(4 * pow(x[0], 2) + pow(x[1], 2) - 3 * x[0] * x[1] +
            2 * pow(x[2], 2) + 5 * x[5] - 11 * x[6]);
  x[7] = pow((x[0] - 10), 2) + 5 * pow((x[1] - 12), 2) + pow(x[2], 4) +
...
double prob_3_dim2(double* x) {
  ...
  x[3] = x[0] * x[1] - 0.2;
  x[2] = pow(15 * x[1] - 5.1 / 4 / pi / pi * pow(15 * x[0] - 5, 2) +
                 5 / pi * (15 * x[0] - 5) - 6,
             2) +
