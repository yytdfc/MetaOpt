...
void GKrig::GKinitialize(int      ncorr,
                         int      nconst_theta,
                         int      nporder,
                         int      nnorm,
                         int      ndcmp,
                         ...
                         double*  up,
                         double*  low) {
  initialize(ncorr, nconst_theta, nporder, nnorm, ndcmp, nParaOpt, nregular,
...
double GKrig::GKpredictor(double* vec_interp) {
  if (isGK) {
    ...
    if (porder == 1) {
      for (i = 1; i <= n_dim; i++) {
        phi[i] = fxIndex(vec_interp, yIndex) * vec_interp[i - 1];
      }
    }
  }
  init_phi = 0;
