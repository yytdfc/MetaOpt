...
void GA::setFx(FUNC f){
	fx=f;
}
void GA::GAinit(int n, int Pop, int Gens, int Cons, double pCr, double pMu,double *up,double *low) {
	int i;
	nPop = Pop;
	nGens = Gens;
	nVar = n;
...
void GA::SBXover(int k){
	...
	for (i = 0; i < nVar; ++i){
		if (randomDouble() < pCrossover){
			if (newpop[k][i] > newpop[k + 1][i])
				swap(newpop[k][i], newpop[k + 1][i]);
			...
			beta = 1 + 2 / dif * min(newpop[k][i] - lower[i],
				upper[i] - newpop[k + 1][i]);
			...
			if (newpop[k][i] > upper[i])
				newpop[k][i] = upper[i];
			if (newpop[k][i] < lower[i])
				newpop[k][i] = lower[i];
			if (isnan(newpop[k + 1][i]) || isinf(newpop[k + 1][i]))
				newpop[k][i] = randomDouble(lower[i], upper[i]);
			if (newpop[k + 1][i] > upper[i])
				newpop[k + 1][i] = upper[i];
			if (newpop[k + 1][i] < lower[i])
				newpop[k + 1][i] = lower[i];
			++nCrossover;
		}
	}
}
...
void GA::PBMutation(int k){
	...
	for (i = 0; i < nVar; ++i){
		if (randomDouble() < pMutation){
			...
			delta = min(newpop[k][i] - lower[i], upper[i] - newpop[k][i])
				/ deltamax;
			if (u > 0.5){
				u = 1 - u;
