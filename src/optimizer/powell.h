...
class Powell
{
 public:
  ...
  int evolve(double*  x0,
             double** direc,
             int      maxiter,
             double   ftol,
             int      dispIter = 0,
             double   terminalLine = -10000);
  int Optimize(double p[],
               double ftol,
               int    maxiter = 1000,
               int    dispIter = 0,
               double terminalLine = -10000);
 ...
 private:
  ...
  inline double sgn(double x) { return x < 0 ? -1 : (x == 0 ? 0 : 1); }
};
