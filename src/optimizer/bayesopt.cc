...
BayesOpt<Real>::BayesOpt(const int               n_x,
         const int               n_obj,
         const int               n_con,
  ...
  ga_.reset(new Ga<Real>(n_x, n_obj, n_con, nullptr, lower, upper));
};

template <typename Real>
void BayesOpt<Real>::opt(){
  samples_ = Doe<Real, LHS>::gen(10, this->n_x_, this->lower_, this->upper_, this->n_obj_, this->n_con_);
  for(auto& s: samples_)
    this->evaluate(s);
  std::vector<Real> t(0);
