...
void Kriging::initialize(int ncorr, int nconst_theta, int nporder,
	int nnorm, int ndcmp, int nParaOpt, int nregular, int ndim,
	int npoints, int nout_points, int nny, double **xxx, double *up, double *low){
	int i, j, k;
	rho_flag = 0;
	cokrig = 0;
	corr = ncorr;
...
void Kriging::readInput(string infile){
	...
	for (i = 0; i<n_dim; i++){
		...
		if (xbound[i][0] > xbound[i][1])
			swap(xbound[i][0] , xbound[i][1]);
	}
	...
	cout << restart << "\t" << corr << "\t" << const_theta << "\t" << porder << endl;
...
void Kriging::fitting(){
	...
	| Here only for porder = 0;for porder = 1,it will update for each interpolation
	----------------------------------------------------------------------------*/
	...
	for (i = 0; i < points; i++)
	{
		...
		else if (flag[i] > 0 || flag[i] < -1)
		{
			if (flag[i] > 0)
				...
				k = abs(flag[i]) - 2;
			vstar[i] = cross_correlation_of_r
				(&xx[i][0], &vec_interp[0], k
				);
		}
	}
...
double Kriging::grad_predictor(double *vec_interp, int k)
{
	...
	for (i = 0; i < points; i++)
	{
		if (flag[i] == 0 || flag[i] == -1)
		{
			...
			if (k - 1 == l){
				vstar[i] = cross_correlation_of_r2
					(&vec_interp[0], &xx[i][0], k - 1);
			}
			else{
				vstar[i] = cross_correlation_of_r22
					(&vec_interp[0], &xx[i][0], k - 1, l);
			}
		}
	}
...
double Kriging::MSE(double *vec_interp)
{
	...
	----------------------------------------------------------------------------*/
	double variance_r0 = correlation_of_r
		(
		&vec_interp[0], &vec_interp[0]
		);
	...
	for (i = 0; i < points; i++)
	{
		for (j = i; j < points; j++)
		{
				...
				|| (flag[i] == 0 && flag[j] == -1))
			{
				...
				|| (flag[i] < -1 && flag[j] == -1))
			{
				v[i][j] = v[j][i] = cross_correlation_of_r
					(
					&xx[i][0], &xx[j][0], k
					);
			}
				...
				|| (flag[i] < -1 && flag[j]> 0))
			{
				if (k == l)
				{
					v[i][j] = v[j][i] = cross_correlation_of_r2
						(
						&xx[i][0], &xx[j][0], k
						);
				}
				else
					v[i][j] = v[j][i] = cross_correlation_of_r22
					(
					&xx[i][0], &xx[j][0], k, l
					);
			} /* else if( flag[i]>0&& flag[j]>0) */
		...
		for (i = 0; i< n_dim; i++)
		{
			...
			case 3:
				...
				if (kesi >= 0.0&&kesi <= 0.4)
					...
					temp *= (5.0 / 3.0) + kesi*((-20.0 / 3.0) + kesi*(10.0 + kesi*((-20.0 / 3.0) + kesi*(5.0 / 3.0))));
				else
					temp *= 0.0;
				break;
			default:
...
double Kriging::cross_correlation_of_r(double *x1, double *x2, int k)
{
	...
	for (i = 0; i < n_dim; i++)
	{
		...
		case 3:
			...
			{
				if (kesi >= 0.0&&kesi <= 0.4)
					temp *= _theta*(kesi*(-30.0 + kesi*(105.0 + kesi*(-97.5))))*sign(xi - xj);
				else if (kesi>0.4&&kesi<1)
					temp *= _theta*((-20.0 / 3.0) + kesi*(20.0 + kesi*(-20.0 + kesi*(20.0 / 3.0))))*sign(xi - xj);
				else
					temp *= 0.0;
			}
			else
...
* Date   : 29.06.2009
*******************************************************************************/
double Kriging::cross_correlation_of_r2(
	double *x1, double *x2, int k)
{
	...
	for (i = 0; i < n_dim; i++)
	{
		...
		case 1:
			if (i == k)
				temp *= -2.0*_theta*(2.0*_theta*(xi - xj)*(xi - xj) - 1.0)*exp(-_theta*r*r);
			else
				temp *= exp(-_theta*r*r);
			break;
		case 2:
...
void Kriging::cholbksb(double **a, int n, double diag[], double b[],
	double x[])
{
	int i, k;
	double dum;
...
void Kriging::ncholdcmp(double **a, int n, double diag[])
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{
			sum = a[i][j];
			for (k = i - 1; k >= 0; k--) sum -= a[i][k] * a[j][k] * diag[k];
			if (i == j)
			{
				if (sum == 0.0)
				{
...
void Kriging::ncholbksb(double **a, int n, double diag[], double b[],
	double x[])
{
	int i, k;
	double dum;
